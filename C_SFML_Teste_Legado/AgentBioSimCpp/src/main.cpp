...
int main(const int argc, char* argv[])
{
    try
    {
        ...
        for (int index = 1; index < argc; ++index)
        {
            ...
            else if (argument == "--phase31-selftest")
            {
                runPhase31Validation = true;
            }
            else if (argument == "--phase32-selftest")
            {
                runPhase32Validation = true;
            }
        ...
        if (runPhase32Checksum)
        {
            std::cout << agentbiosim::systems::runPhase32StateChecksum();
            return 0;
        }

        if (runPhase32Validation)
        {
            const auto summary = agentbiosim::systems::runPhase32Validation();
            std::cout << "Phase32 validation: " << (summary.passed ? "PASS" : "FAIL")
                      << " (" << summary.checks << " checks)\n"
                      << summary.details << '\n';
        ...
        {
            const auto summary = agentbiosim::systems::runPhase31Validation();
            std::cout << "Phase31 validation: " << (summary.passed ? "PASS" : "FAIL")
                      << " (" << summary.checks << " checks)\n"
                      << summary.details << '\n';
            return summary.passed ? 0 : 31;
        }
