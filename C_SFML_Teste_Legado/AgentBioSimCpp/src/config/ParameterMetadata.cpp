...
const std::vector<LabelEntry> kFriendlyLabels{
    ...
    {"food_chunk_particles",         "Particulas por migalha", "Particles per crumb"},
...
const std::vector<HelpEntry> kParameterHelp{
    ...
    {"food_chunk_mode",
     "So no modo 'em pedacos'. 'Fixo': os pedacos repoem sempre no mesmo lugar (os organismos tendem a acampar). 'Itinerante': as migalhas caem inteiras, sao devoradas e novas migalhas caem em outros lugares, entao os organismos precisam procurar comida.",
     "Only in 'chunks' mode. 'Fixed': chunks always refill in the same place (organisms tend to camp). 'Roaming': whole crumbs fall, get devoured, and new crumbs fall elsewhere, so organisms must search for food."},
    {"food_chunk_particles",
     "So no modo itinerante. Quantas particulas formam UMA migalha inteira. As migalhas caem sempre com essa quantidade (com bordas irregulares); o numero de migalhas no mundo = alvo de comida / particulas por migalha. Valores maiores = poucas migalhas grandes; menores = muitas migalhas pequenas.",
     "Roaming mode only. How many particles make up ONE whole crumb. Crumbs always fall with this many particles (with irregular edges); the number of crumbs in the world = food target / particles per crumb. Larger = few big crumbs; smaller = many small crumbs."},
    {"food_target",
